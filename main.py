...
if __name__ == "__main__":
    ...
    spacex_launch_photos = spacex.fetch_launch(None)["links"]["flickr"]["original"]
    if spacex_launch_photos:
        for image in spacex_launch_photos:
            fstream.download_image(
                image, all_images / f"spx_{fstream.get_filename_from_url(image)}"
            )
    ...
    if nasa_apod:
        for image in nasa_apod:
            fstream.download_image(
                image, all_images / f"apod_{fstream.get_filename_from_url(image)}"
            )
    nasa_epic = epic.get_epics(nasa_api_key)
    if nasa_epic:
        for image in nasa_epic:
            fstream.download_image(
                image, all_images / f"epic_{fstream.get_filename_from_url(image)}"
            )
